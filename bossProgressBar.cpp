...
HRESULT bossProgressBar::init(int x, int y, const char* frontImage, int frontWidth, int frontHeight, const char* backImage, int backWidth, int backHeight)
{
	_x = x;
	_y = y;
...
void bossProgressBar::render()
{
	...
	IMAGEMANAGER->render(_frontImage, CAMERAMANAGER->getCameraDC(), _rcFront.left + _progressBarFront->getWidth() / 2, _y + _progressBarFront->getHeight() / 2 + 37, 0, 0, _width, _progressBarFront->getHeight());
